...
public final class ClassContainerDescriptor extends ClassDescriptor {
    ...
    private void validate() {
        if (this.classes != null && this.classes.size() >= 1) {
            ...
            for (final ClassDescriptor classDescriptor : this.classes) {
                Preconditions.checkArgument(this.getPackageName().equals(classDescriptor.getPackageName()), "The class %s don't have the same package of this class %s, you must a package.", classDescriptor.getImplementationName(), this.classes.get(0).getImplementationName());
            }
        }
    }
